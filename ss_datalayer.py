...
class PairLoaderProcess(Process):
    ...
    def run(self):
        try:
            while True:
                item = None
                while item is None:
                    item, info = self.load_next_frame()
                self.queue.put((item, info))
        except:
    ...
    def load_next_frame(self, try_mode=True):
        ...
        item, info = self.load_frame(*next_pair)
    ...
    def try_some_more(self, max_tries):
        ...
        while(item is None and i<max_tries):
            item, info = self.load_next_frame(True)
            i+=1
    ...
    def load_frame(self, player, first_index, second_index):
        ...
        def get_id(index):
            img_id = player.img_ids[index]
            img_path = player.video_item.read_img(img_id)[1]
            img_id = img_path.split('/')[-1][:-4]

            return img_id

        query_img_id = get_id(second_index)
        support_img_ids = [get_id(x) for x in first_index]

        second_semantic_labels = self.__get_deploy_info(player, second_index)[0]
        ...
        return item, (query_img_id, support_img_ids, int(second_semantic_labels[0]))
...
class SSDatalayer():
    def __init__(self, test_group, k_shot=1):
        ...
        self.param_str = {
            ...
            'has_cont':False,
            'deploy_mode':True,
            'pascal_path':'./datasets/VOCdevkit/VOC2012'
        }
